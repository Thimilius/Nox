...
when TRACY_ENABLE {
	...
	when ODIN_OS == .Windows {
		...
		*/
		...
		@(disabled=!TRACY_ENABLE) ZoneC  :: #force_inline proc(color: u32, active := true, depth: i32 = TRACY_CALLSTACK, loc := #caller_location) { }
		@(disabled=!TRACY_ENABLE) SetThreadName :: #force_inline proc(name: cstring) {  }
		
	}

} else {

	@(disabled = true) ZoneC :: proc(color: u32) {}
